...
Dir[SPEC_ROOT.join('support/unit/*.rb').to_s].sort.each { |path| require(path) }

require SPEC_ROOT.join('../system/api/container')

I18n.config.load_path += Dir[Api::Container.root.join('spec', 'locals', '*.yml').to_s]

RSpec.configure do |config|
  config.include Test::Request::MockConfig, type: :vcr

  config.disable_monkey_patching!

  config.around do |example|
    Api::Container.enable_stubs!
